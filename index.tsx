...
function RapidCutEditor() {
  ...
  }, [activeProjectId, projectName, items, projectSettings, library]);
  ...
  const handleImport = async (files: FileList) => {
    ...
    for (const file of Array.from(files)) {
      ...
      const asset = { name: file.name, url, duration: dur, type: isAudio ? 'audio' : 'video' } as MediaAsset;
    ...
    <div className="flex flex-col h-screen bg-[#0d0d0f] text-zinc-400 font-sans overflow-hidden select-none text-[11px]">
      ...
      <main className="flex-1 flex overflow-hidden min-h-0">
        <MediaBin 
          ...
          onAddFromLibrary={(a) => addItem({ id: Math.random().toString(), type: a.type, startTime: internalTimeRef.current, duration: a.duration, trimStart: 0, originalDuration: a.duration, name: a.name, url: a.url, color: a.type === 'video' ? 'bg-zinc-700' : 'bg-emerald-600', muted: false, volume: 1.0 })} 
