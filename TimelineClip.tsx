...
import { Type, Music, Volume2, VolumeX, Sparkles } from 'lucide-react';
...
export const TimelineClip = memo(({ 
  ...
  const hasTransition = item.transition && item.transition !== 'none';
  ...
  return (
    ...
    >
      ...
      >
        ...
        {hasTransition && (
          <div 
            className="absolute left-0 top-0 bottom-0 bg-indigo-500/30 backdrop-blur-[2px] z-10 pointer-events-none flex items-center justify-center border-r border-indigo-400/50"
            style={{ width: TRANSITION_DUR * pxPerSec }}
          >
            <Sparkles size={10} className="text-indigo-200 animate-pulse" />
          </div>
        )}
